...
*/
const elemsV = document.getElementsByClassName('lesson__item-list');
console.log(elemsV);
...
const lessonListV = document.querySelector('.lesson__list');
lessonListV.insertAdjacentHTML(
   "beforeend",
   '<li class="lesson__item-list">New point</li>'
);
...
*/
const elemV = document.querySelector('.lesson__item-sub-list');
const parentList = elemV.closest('.lesson__list');
...
*/
const elemsV1 = document.querySelectorAll('.lesson__item-list');
for(let elem of elems){
   if (elem.matches('[class$="lesson__item-list_red"]')) {
      console.log('Red');
   } else if(elem.matches('[class$="lesson__item_blue"]')){
      console.log('Blue');
   }
}
...
// Получаем объект
const textElementV = document.querySelector('.lesson__text');
// Получаем содержимое объекта "как есть"
// вместе с HTML, а также сам элемент
const textElementContentV = textElementV.outerHTML;
console.log(textElementContentV);

textElementV.outerHTML = `<p>Живи, а работай в 
   <span class="yellow">свободное</span> время!</p>`;
console.log(textElementV.outerHTML);
...
//...перед объектом
textElement1.before(newElement1);
//...после объекта
textElement1.after(newElement1);
//...внутрь и в конец объекта
textElement1.append(newElement1);

// Вставка нескольких фрагментов сразу
textElement1.append(newElement1, "Привет!");

// Можно вставлять строку
textElement1.append(`Живи, а работай в
   <span class="yellow">свободное</span> время!`);
/*
...
textElement3.insertAdjacentHTML(
   'afterend',
   `<p>Живи, а работай в
	<span class="yellow">свободное</span> время!</p>`
);
...
// Вставляем текст
textElement4.insertAdjacentText(
   'beforeend',
   `Живи, а работай в
	<span class="yellow">свободное</span> время!`
)
...
// Наполняем новый элемент
newElement2.innerHTML = `Живи, а работай в
   <span class="yellow">свободное</span> время!`;
// Вставляем элемент
textElement4.insertAdjacentElement(
   'beforeend',
   newElement2
);
/*
...
const link = document.querySelector('.lesson__input');
